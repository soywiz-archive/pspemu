...
uint getCommonMask(Instruction[] ilist, uint _mask = 0x_FFFFFFFF) {
	uint mask = _mask;
	foreach (i; ilist) mask &= i.mask;
	return mask;
}
/*
uint getCommonValues(immutable Instruction[] ilist, uint mask = 0x_FFFFFFFF) {
}*/

bool inInstruction(uint[] l, uint v) {
	foreach (c; l) if (c == v) return true;
	return false;
}

// Generate a set of switch for decoding instructions.
string genSwitch(string prefix, Instruction[] ilist, uint _mask = 0x_FFFFFFFF, int level = 0) {
	string r = "";
	...
	if (ilist.length == 0) {
		...
		r = indent(level, prefix ~ process_name(ilist[0].name) ~ "(o); return;\n");
	} if (ilist.length > 1) {
		Instruction[512] ci; int ci_len = ilist.length;

		uint[] cvalues;
		//inInstruction(cmask, 0);

		uint mask = getCommonMask(cast(Instruction[])ilist, _mask);
		r ~= indent(level + 0, "switch (o.v & " ~ getString(mask) ~ ") {\n");
		foreach (i; ilist) {
			uint cvalue = i.opcode & mask; if (inInstruction(cvalues, cvalue)) continue;
		...
		r ~= indent(level + 1, "default: OP_UNK(o); return;\n");
		r ~= indent(level + 0, "}\n");
	}
	return r;
}
