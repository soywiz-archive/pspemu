...
private T enforce(T)(T v, string file = __FILE__, int line = __LINE__) {
	if (FAILED(v)) throw(new Exception(std.string.format("Failed at '%s':%d", file, line)));
	return v;
}

public void convertOmaToWav(string source, string destination) {
	return convertOmaToWav(
		std.utf.toUTF16(source),
		std.utf.toUTF16(destination)
	);
}

public void convertOmaToWav(wstring source, wstring destination) {
