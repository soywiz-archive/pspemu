...
void checkChangePathsTry(const char *dest) {
	if (chdir(dest) < 0) {
		emitString("(chdir error)");
	} else {
		emitString(getcwd(buf, MAXPATHLEN) ?: "(getcwd error)");
	}
}
...
void checkChangePaths() {
	emitString(getcwd(buf, MAXPATHLEN));
	...
	while (nfiles < MAX_ENTRY) {
		...
		if (strcmp(files[nfiles].d_name, "GAME") == 0) {
			...
			if (files[nfiles].d_stat.st_attr == FIO_SO_IFDIR){
				emitString(files[nfiles].d_name);
				continue;
			}
		}
	}
...
void checkMainArgs(int argc, char** argv) {
	int n;
	emitInt(argc);
	for (n = 0; n < argc; n++) emitString(argv[n]);
}
