...
version (Windows) {
	...
	template WinGl_Imports() {
		void* function(const char*) wglGetProcAddress;
	...
	void BindLibraryGl(alias bindTemplate, bool check = true)() {
		BindLibraryFunctions!(q{
		}, q{
			{ static if (__traits(compiles, &{$dname})) {
				void* addr = pspemu.utils.OpenGL.wglGetProcAddress(cast(char*)"{$importName}");
				if (check) if (addr is null) throw(new Exception(std.string.format("Can't bind '%s' : '%s'", "{$importName}", std.windows.syserror.sysErrorString(std.c.windows.windows.GetLastError))));
				*cast(void**)&{$dname} = addr;
			} }
		}, "", bindTemplate, "pspemu.utils.OpenGL.", "", check);
	}

	private void glInitWindows() {
		BindLibrary!("opengl32.dll", WinGl_Imports, "pspemu.utils.OpenGL.");
	}
} else {
	static assert (0 != 0);
}
...
class glProgram {
	...
	glAttrib getAttrib(string name, bool except = false) {
		GLuint aid = glGetAttribLocation(id, cast(char*)std.string.toStringz(name));
	...
	glUniform getUniform(string name, bool except = false) {
		GLuint aid = glGetUniformLocation(id, cast(char*)std.string.toStringz(name));
		if (except && aid == -1) throw(new Exception(std.string.format("Attribute '%s' isn't active", name)));
...
template OpenglWinExt_Template() {
	bool function(int) wglSwapIntervalEXT;
}

mixin Opengl11_Imports;
mixin Opengl12_Imports;
mixin Opengl13_Imports;
mixin Opengl14_Imports;
mixin Opengl15_Imports;
mixin Opengl20_Imports;
mixin Opengl21_Imports;
mixin OpenglWinExt_Template;

void glUsing11() { BindLibrary!("opengl32.dll", Opengl11_Imports, "pspemu.utils.OpenGL."); }
void glUsing12() { BindLibraryGl!(Opengl12_Imports); }
void glUsing13() { BindLibraryGl!(Opengl13_Imports); }
void glUsing14() { BindLibraryGl!(Opengl14_Imports); }
void glUsing15() { BindLibraryGl!(Opengl15_Imports); }
void glUsing20() { BindLibraryGl!(Opengl20_Imports); }
void glUsing21() { BindLibraryGl!(Opengl21_Imports); }
void glUsing30() { }
void glUsing31() { }
void glUsing32() { }
void glUsing33() { }
