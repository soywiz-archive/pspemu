...
/**
 * 16bit IEEE HalfFloat
 */
alias CustomFloat!(10, 5, CustomFloatFlags.ieee) hfloat;

hfloat ushortToHfloat(uint v) {
	hfloat hv;
	hv.significand = BitUtils.extract!(uint, 0, 10)(v);
	hv.exponent    = BitUtils.extract!(uint, 10, 5)(v);
	hv.sign        = cast(bool)BitUtils.extract!(uint, 15, 1)(v);
	return hv;
}
...
struct Instruction {
	 ...
	 */
	@property float IMM_HF() { return cast(float)ushortToHfloat(IMMU); }
