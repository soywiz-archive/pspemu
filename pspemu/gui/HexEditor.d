...
import pspemu.gui.HexEditorUtils;
...
class HexEditorComponent : UserControl {
	...
	Color colorAddress, colorHex, colorText;
	...
	void showSearchForm() {
		...
		searchForm.wparent = this;
		searchForm.show();
	}

	void showGotoForm() {
		if (gotoForm is null || !gotoForm.visible) gotoForm = new GotoForm();
		gotoForm.wparent = this;
		gotoForm.updateGoto();
		gotoForm.show();
	}
	...
	this() {
		...
		with (new Timer) {
			...
			tick ~= (Timer sender, EventArgs ea) {
				//static int n = 0; writefln("tick: %d, %s", n++, updatedPosition);
				if (updatedPosition) {
	...
	override void onMouseWheel(MouseEventArgs ea) { super.onMouseWheel(ea); }
