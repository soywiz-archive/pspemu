...
/* PI, float-sized */
const float GU_PI	= 3.141593f;

/* Boolean values for convenience */
const int GU_FALSE = 0;
const int GU_TRUE	= 1;

/* Primitive types */
const int GU_POINTS = 0;
const int GU_LINES = 1;
const int GU_LINE_STRIP = 2;
const int GU_TRIANGLES = 3;
const int GU_TRIANGLE_STRIP = 4;
const int GU_TRIANGLE_FAN = 4;
const int GU_SPRITES = 6;

/* States */
const int GU_ALPHA_TEST = 0;
const int GU_DEPTH_TEST = 1;
const int GU_SCISSOR_TEST = 2;
const int GU_STENCIL_TEST = 3;
const int GU_BLEND = 4;
const int GU_CULL_FACE = 5;
const int GU_DITHER = 6;
const int GU_FOG = 7;
const int GU_CLIP_PLANES = 8;
const int GU_TEXTURE_2D = 9;
const int GU_LIGHTING = 10;
const int GU_LIGHT0 = 11;
const int GU_LIGHT1 = 12;
const int GU_LIGHT2 = 13;
const int GU_LIGHT3 = 14;
const int GU_LINE_SMOOTH = 15;
const int GU_PATCH_CULL_FACE = 16;
const int GU_COLOR_TEST = 17;
const int GU_COLOR_LOGIC_OP = 18;
const int GU_FACE_NORMAL_REVERSE = 19;
const int GU_PATCH_FACE = 20;
const int GU_FRAGMENT_2X = 21;

/* Matrix modes */
const int GU_PROJECTION = 0;
const int GU_VIEW	 = 1;
const int GU_MODEL = 2;
const int GU_TEXTURE = 3;

/* Vertex Declarations Begin */
int GU_TEXTURE_SHIFT(int n)
{
	return ((n)<<0);
}
const int GU_TEXTURE_8BIT = GU_TEXTURE_SHIFT(1);
const int GU_TEXTURE_16BIT	= GU_TEXTURE_SHIFT(2);
const int GU_TEXTURE_32BITF = GU_TEXTURE_SHIFT(3);
const int GU_TEXTURE_BITS = GU_TEXTURE_SHIFT(3);

int GU_COLOR_SHIFT(int n)	
{
	return ((n)<<2);
}
const int GU_COLOR_5650 = GU_COLOR_SHIFT(4);
const int GU_COLOR_5551 = GU_COLOR_SHIFT(5);
const int GU_COLOR_4444 = GU_COLOR_SHIFT(6);
const int GU_COLOR_8888 = GU_COLOR_SHIFT(7);
const int GU_COLOR_BITS = GU_COLOR_SHIFT(7);

int GU_NORMAL_SHIFT(int n)
{
	return ((n)<<5);
}
const int GU_NORMAL_8BIT = GU_NORMAL_SHIFT(1);
const int GU_NORMAL_16BIT = GU_NORMAL_SHIFT(2);
const int GU_NORMAL_32BITF = GU_NORMAL_SHIFT(3);
const int GU_NORMAL_BITS = GU_NORMAL_SHIFT(3);

int GU_VERTEX_SHIFT(int n)
{
	return ((n)<<7);
}
const int GU_VERTEX_8BIT = GU_VERTEX_SHIFT(1);
const int GU_VERTEX_16BIT = GU_VERTEX_SHIFT(2);
const int GU_VERTEX_32BITF = GU_VERTEX_SHIFT(3);
const int GU_VERTEX_BITS = GU_VERTEX_SHIFT(3);

int GU_WEIGHT_SHIFT(int n)
{
	return ((n)<<9);
}
const int GU_WEIGHT_8BIT = GU_WEIGHT_SHIFT(1);
const int GU_WEIGHT_16BIT = GU_WEIGHT_SHIFT(2);
const int GU_WEIGHT_32BITF = GU_WEIGHT_SHIFT(3);
const int GU_WEIGHT_BITS = GU_WEIGHT_SHIFT(3);

int GU_INDEX_SHIFT(int n)
{
	return ((n)<<11);
}
const int GU_INDEX_8BIT = GU_INDEX_SHIFT(1);
const int GU_INDEX_16BIT = GU_INDEX_SHIFT(2);
const int GU_INDEX_BITS = GU_INDEX_SHIFT(3);

int GU_WEIGHTS(int n)
{
	return ((((n)-1)&7)<<14);
}
const int GU_WEIGHTS_BITS = GU_WEIGHTS(8);
int GU_VERTICES(int n)
{
	return ((((n)-1)&7)<<18);
}
const int GU_VERTICES_BITS = GU_VERTICES(8);

int GU_TRANSFORM_SHIFT(int n)
{
	return ((n)<<23);
}
const int GU_TRANSFORM_3D = GU_TRANSFORM_SHIFT(0);
const int GU_TRANSFORM_2D = GU_TRANSFORM_SHIFT(1);
const int GU_TRANSFORM_BITS = GU_TRANSFORM_SHIFT(1);
/* Vertex Declarations End */

/* Pixel Formats */
const int GU_PSM_5650 = 0; /* Display, Texture, Palette */
const int GU_PSM_5551 = 1; /* Display, Texture, Palette */
const int GU_PSM_4444 = 2; /* Display, Texture, Palette */
const int GU_PSM_8888 = 3; /* Display, Texture, Palette */
const int GU_PSM_T4 = 4; /* Texture */
const int GU_PSM_T8 = 5; /* Texture */
const int GU_PSM_T16 = 6; /* Texture */
const int GU_PSM_T32 = 7; /* Texture */
const int GU_PSM_DXT1 = 8; /* Texture */
const int GU_PSM_DXT3 = 9; /* Texture */
const int GU_PSM_DXT5 = 10; /* Texture */

/* Spline Mode */
const int GU_FILL_FILL = 0;
const int GU_OPEN_FILL = 1;
const int GU_FILL_OPEN = 2;
const int GU_OPEN_OPEN = 3;

/* Shading Model */
const int GU_FLAT = 0;
const int GU_SMOOTH = 1;

/* Logical operation */
const int GU_CLEAR = 0;
const int GU_AND = 1;
const int GU_AND_REVERSE = 2;
const int GU_COPY = 3;
const int GU_AND_INVERTED = 3;
const int GU_NOOP = 4;
const int GU_XOR = 6;
const int GU_OR = 7;
const int GU_NOR = 8;
const int GU_EQUIV = 9;
const int GU_INVERTED = 10;
const int GU_OR_REVERSE = 11;
const int GU_COPY_INVERTED = 12;
const int GU_OR_INVERTED = 13;
const int GU_NAND = 14;
const int GU_SET = 15;

/* Texture Filter */
const int GU_NEAREST = 0;
const int GU_LINEAR = 1;
const int GU_NEAREST_MIPMAP_NEAREST = 4;
const int GU_LINEAR_MIPMAP_NEAREST = 5;
const int GU_NEAREST_MIPMAP_LINEAR = 6;
const int GU_LINEAR_MIPMAP_LINEAR = 7;

/* Texture Map Mode */
const int GU_TEXTURE_COORDS = 0;
const int GU_TEXTURE_MATRIX = 1;
const int GU_ENVIRONMENT_MAP = 2;

/* Texture Level Mode */
const int GU_TEXTURE_AUTO = 0;
const int GU_TEXTURE_CONST = 1;
const int GU_TEXTURE_SLOPE = 2;

/* Texture Projection Map Mode */
const int GU_POSITION = 0;
const int GU_UV = 1;
const int GU_NORMALIZED_NORMAL = 2;
const int GU_NORMAL = 3;

/* Wrap Mode */
const int GU_REPEAT = 0;
const int GU_CLAMP = 1;
...
/* Test Function */
const int GU_NEVER = 0;
const int GU_ALWAYS = 1;
const int GU_EQUAL = 2;
const int GU_NOTEQUAL = 3;
const int GU_LESS = 4;
const int GU_LEQUAL = 5;
const int GU_GREATER = 6;
const int GU_GEQUAL = 7;

/* Clear Buffer Mask */
const int GU_COLOR_BUFFER_BIT = 1;
const int GU_STENCIL_BUFFER_BIT = 2;
const int GU_DEPTH_BUFFER_BIT = 4;
const int GU_FAST_CLEAR_BIT = 16;

/* Texture Effect */
const int GU_TFX_MODULATE = 0;
const int GU_TFX_DECAL = 1;
const int GU_TFX_BLEND = 2;
const int GU_TFX_REPLACE = 3;
const int GU_TFX_ADD = 4;

/* Texture Color Component */
const int GU_TCC_RGB = 0;
const int GU_TCC_RGBA = 1;

/* Blending Op */
const int GU_ADD = 0;
const int GU_SUBTRACT = 1;
const int GU_REVERSE_SUBTRACT = 2;
const int GU_MIN = 3;
const int GU_MAX = 4;
const int GU_ABS = 5;

/* Blending Factor */
const int GU_SRC_COLOR = 0;
const int GU_ONE_MINUS_SRC_COLOR = 1;
const int GU_SRC_ALPHA = 2;
const int GU_ONE_MINUS_SRC_ALPHA = 3;
const int GU_DST_COLOR = 0;
const int GU_ONE_MINUS_DST_COLOR = 1;
const int GU_DST_ALPHA = 4;
const int GU_ONE_MINUS_DST_ALPHA = 5;
const int GU_FIX = 10;

/* Stencil Operations */
const int GU_KEEP = 0;
const int GU_ZERO = 1;
const int GU_REPLACE	= 2;
const int GU_INVERT = 3;
const int GU_INCR = 4;
const int GU_DECR = 5;

/* Light Components */
const int GU_AMBIENT	 = 1;
const int GU_DIFFUSE = 2;
const int GU_SPECULAR = 4;
const int GU_AMBIENT_AND_DIFFUSE	= (GU_AMBIENT|GU_DIFFUSE);
const int GU_DIFFUSE_AND_SPECULAR = (GU_DIFFUSE|GU_SPECULAR);
const int GU_UNKNOWN_LIGHT_COMPONENT = 8;

/* Light modes */
const int GU_SINGLE_COLOR = 0;
const int GU_SEPARATE_SPECULAR_COLOR = 1;

/* Light Type */
const int GU_DIRECTIONAL = 0;
const int GU_POINTLIGHT = 1;
const int GU_SPOTLIGHT = 2;

/* Contexts */
const int GU_DIRECT = 0;
const int GU_CALL = 1;
const int GU_SEND = 2;
...
/* Sync behavior (mode) */
const int GU_SYNC_FINISH = 0;
const int GU_SYNC_SIGNAL = 1;
const int GU_SYNC_DONE = 2;
const int GU_SYNC_LIST = 3;
const int GU_SYNC_SEND = 4;

/* behavior (what) */
const int GU_SYNC_WAIT = 0;
const int GU_SYNC_NOWAIT = 1;

/* Sync behavior (what) [see pspge.h] */
const int GU_SYNC_WHAT_DONE = 0;
const int GU_SYNC_WHAT_QUEUED = 1;
const int GU_SYNC_WHAT_DRAW = 2;
const int GU_SYNC_WHAT_STALL = 3;
const int GU_SYNC_WHAT_CANCEL = 4;

/* Signals */
const int GU_CALLBACK_SIGNAL = 1;
const int GU_CALLBACK_FINISH = 4;

/* Signal behavior */
const int GU_BEHAVIOR_SUSPEND = 1;
const int GU_BEHAVIOR_CONTINUE = 2;

/* Color Macros, maps 8 bit unsigned channels into one 32-bit value */
int GU_ABGR(float a, float b, float g, float r)	
{
	return cast(int)(((cast(int)a) << 24)|((cast(int)b) << 16)|((cast(int)g) << 8)|(cast(int)r));
}
int GU_ARGB(float a, float r, float g, float b)	
{
	return GU_ABGR((a),(b),(g),(r));
}
int GU_RGBA(float r, float g, float b, float a)
{
	return GU_ARGB((a),(r),(g),(b));
}

/* Color Macro, maps floating point channels (0..1) into one 32-bit value */
uint GU_COLOR(float r, float g, float b, float a)
{
	return GU_RGBA((r * 255.0f), (g * 255.0f), (b * 255.0f), (a * 255.0f));
}

alias void (*GuSwapBuffersCallback)(void** display,void** render);
...
extern (C)
{
	...
	**/
	void sceGuDepthFunc(int); //int function
	...
	**/
	...
	void sceGuDepthOffset(uint offset);
	...
	**/
	...
	void sceGuFog(float near, float far, uint color);
	...
	**/
	void sceGuCallList(void* list); //const void*
	...
	**/
	void sceGuSendList(int mode, void* list, PspGeContext* context);
	...
	**/
	void sceGuDrawArray(int prim, int vtype, int count, void* indices, void* vertices);
	...
	/**
	  ...
	  * @param vertices - Pointer to a vertex-list
	**/
	void sceGuBeginObject(int vtype, int count, void* indices, void* vertices);
	...
	**/
	void sceGuLight(int light, int type, int components, ScePspFVector3* position);
	...
	**/
	void sceGuLightColor(int light, int component, uint color);
	...
	**/
	void sceGuLightSpot(int light, ScePspFVector3* direction, float exponent, float cutoff);
	...
	**/
	void sceGuClearColor(uint color);
	...
	**/
	void sceGuClearDepth(uint depth);
	...
	/**
	  ...
	  *
	**/
	void sceGuClearStencil(uint stencil);
	...
	**/
	void sceGuColorFunc(int func, uint color, uint mask);
	...
	**/
	...
	void sceGuAmbient(uint color);
	void sceGuAmbientColor(uint color);
	...
	**/
	void sceGuBlendFunc(int op, int src, int dest, uint srcfix, uint destfix);
	...
	/**
	  *
	**/
	void sceGuModelColor(uint emissive, uint ambient, uint diffuse, uint specular);
	...
	**/
	void sceGuStencilFunc(int func, int, int mask);
	...
	**/
	void sceGuSetDither(ScePspIMatrix4* matrix);
	...
	**/
	...
	/**
	  ...
	  * @code
	  * sceGuCopyImage(GU_PSM_8888,0,0,480,272,512,pixels,0,0,512,(void*)(((uint)framebuffer)+0x4000000));
	  * @endcode
	  *
	...
	**/
	void sceGuTexEnvColor(uint color);
	...
	**/
	void sceGuTexImage(int mipmap, int width, int height, int tbw, void* tbp);
	...
	**/
	void sceGuTexLevelMode(uint mode, float bias);
	...
	/**
	  ...
	  * @param a2 - Unknown
	**/
	void sceGuTexMapMode(int mode, uint a1, uint a2);
	...
	**/
	void sceGuClutLoad(int num_blocks, void* cbp);
	...
	**/
	void sceGuOffset(uint x, uint y);
	...
	**/
	void sceGuDrawBezier(int vtype, int ucount, int vcount, void* indices, void* vertices);
	...
	**/
	void sceGuPatchDivide(uint ulevel, uint vlevel);

	void sceGuPatchFrontFace(uint a0);
	...
	**/
	void sceGuSetMatrix(int type, ScePspFMatrix4* matrix);
	...
	**/
	void sceGuBoneMatrix(uint index, ScePspFMatrix4* matrix);
